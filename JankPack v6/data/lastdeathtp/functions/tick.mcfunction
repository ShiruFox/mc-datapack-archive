...
execute as @e[type=armor_stand,name="deathStand"] run title @a actionbar {"text":"Death teleport available","color":"green","bold":true}
...
execute as @a[scores={deathTP=1}] unless entity @e[type=armor_stand,name="deathStand"] run tellraw @a ["",{"text":"LDTP: ","bold":true,"color":"gold"},{"text":"Error; Couldn't find previous death - Where you teleported once already?","color":"red"}]
execute as @e[type=armor_stand,name="deathStand"] run execute as @a[scores={deathTP=1}] run tellraw @a ["",{"text":"LDTP: ","bold":true,"color":"gold"},"Death location deleted"]
execute as @a[scores={deathTP=1}] run kill @e[type=armor_stand,name="deathStand"]
